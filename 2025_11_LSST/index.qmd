...
![](assets/latest/summ_stat.png){fig-align="center" width="80%" text-align="right" style="border: 2px solid black; border-radius: 6px;" fig-cap="" alt="Credit: Natalia Porqueres"}
::::::
:::::: {.fragment fragment-index=1 .fade-in}
![](assets/latest/bad_posterior.png){fig-align="center" width="50%" style="border: 2px solid black; border-radius: 6px;" fig-cap="" alt="Jeffrey et al. (2024): bad posterior"}
::::::
:::
...
## Born vs Raytrace (z = 0.8)

:::: {.columns}

::: {.column width="65%"}
![](assets/latest/born_vs_raytrace_maxz.png){width=80%}
:::
...
\end{pmatrix}
$$
Deflection angle (with multiple deflections):
$$
\boldsymbol{\alpha}(\boldsymbol{\theta}) = \nabla_\perp \psi(\boldsymbol{\theta}) = \frac{2}{c^2}\! \int_0^{r_s} dr\, \nabla_\perp \Phi(r,\boldsymbol{\theta})
$$
::::
:::
...
The choice of the lensing method can have an impact on the inferred cosmological parameters at LSST precision.
...
:::
:::: {.solutionbox-body style="font-size: 20px;"}
* More expensive (spherical transforms, larger covariances) but keeps the largest-scale modes intact.
* Avoids patch seams; cleaner E/B control at LSST precision.
* If you care about low-$\ell$ stability and uniform systematics, full-sphere wins.
::::
:::
...
- Inference is only as strong as the simulator it rests on.
- Fast is not enough — we need high resolution to capture small-scale physics.
- Weak lensing demands ~1 cell / Mpc/h to keep shapes and power unbiased.

:::

::: {.column width="50%"}
![](assets/latest/likelihood_free_ffi.svg){fig-align="center" width="90%"}
:::
...
## <span style="color:black">JAXPM v0.1.6: Differentiable, Scalable Simulations </span> {background-image="assets/Fields/LPT_density_field_z0_2048.png" background-opacity="0.2" style="font-size: 20px;"}
...
* In the near future we aim to apply this on **DES** data
