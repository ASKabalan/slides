...
## The Impact of inaccurate simulations on Physical Modeling

::: {.columns}

:::: {.column width="50%"}

![](assets/bayes/FFI_full.svg){fig-align="center" width="80%"}
...
Cloud-In-Cell (CIC) requires particles to paint to nearby grid cells — even across subdomain boundaries.  
To avoid loss of mass and ensure correct gradients, we apply **halo exchange** and synchronize the overlapping regions.
...
::: {.solutionbox}
:::: {.solutionbox-body style="font-size: 18px;"}
Halo exchange ensures correct mass deposition and gradient flow across domain boundaries — essential for differentiable simulations.
::::
:::

---

## Why Halo Exchange Matters in Distributed Simulations {style="font-size: 22px;"}
...
## jaxDecomp: Halo Exchange for Distributed Simulations {style="font-size: 22px;"}

[![](https://img.shields.io/badge/GitHub-jaxdecomp-blue?logo=github)](https://github.com/DifferentiableUniverseInitiative/jaxDecomp)

:::{.fragment fragment-index=1}

* Part of the `jaxdecomp` package — **fully differentiable** and **JAX-compatible**
* **Easy to use** and works transparently across **GPUs and nodes**
* Open source and available on **PyPI**
...
![](assets/Fields/CIC/Halo_Exchange.svg){fig-align="center" width="60%"}

:::

# Differentiating Through Time Integration

::: {.rstack}

![](assets/bayes/FFI_full.svg){fig-align="center" width="30%"}

:::

---

## Autodiff in ![](assets/Logos/JaxLogo.png){width="5%"} 101
...
To differentiate it, autodiff systems track:

- The tangents: $\dot{x}$, $\dot{y}$
- The inputs: $x$, $y$
...
### Applied to time integration:

In a simple **Kick-Drift scheme**:
...
To compute gradients, we must store:
...
* This leads to a **linear memory footprint** — each additional step increases memory cost

* Reducing the number of steps leads to **miss estimating small-scale structure**
  → Just like how **low resolution underestimates power on small scales**
...
## The Adjoint Trick: Backprop Without Storing Everything (**Preliminary**)
...
* We use the **reverse adjoint method**:
  ...
  * **Re-integrate backward in time** to get gradients
...
- ✅ Memory stays constant, regardless of the number of steps
- ⚠️ Requires a second pass through the simulation
- For A 10 step 1024³ Full PM Lightcone pipeline, it requires 5 times less memory than the checkpoint approach
...
## Sampling in Differentiable Simulations (**Preliminary**) {style="font-size: 19px;"}


::: {layout-ncol=2}

![Forward Field-Level Model](assets/latest/FFI_split.svg){fig-align="center" width="100%"}

![JAX Sampling Flow](assets/latest/Key_split.svg){fig-align="center" width="100%"}
